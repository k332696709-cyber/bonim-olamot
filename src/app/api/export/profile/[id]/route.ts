...
export async function POST(
  ...
  const type   = req.nextUrl.searchParams.get('type')   ?? 'card'   // 'card' | 'full'
  const locale = (req.nextUrl.searchParams.get('locale') ?? 'he') as 'he' | 'en'

  // TODO: replace with real database lookup
  let doc: ReturnType<typeof buildProfilePdf | typeof buildMaleProfilePdf | typeof buildFullResumePdf | typeof buildMaleFullResumePdf> | null = null
